...
const Item = ({
    ...
    const onCreate = (
        ...
        event.stopPropagation();
        if(!id) return;
        const promice = create({ title: "Untitled", parentDocument: id })
        .then((documentId) =>{
            if(!expanded){
                onExpand?.();
            }
  ...
  return (
    ...
    {!!id && (
        <div role="button" onClick={onCreate} className="ml-auto flex items-center gap-x-2">
            <div className="opacity-0 group-hover:opacity-100 h-full ml-auto rounded-sm hover:bg-neutral dark:hover:bg-neutral-600">
